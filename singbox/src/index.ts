...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		for (const sub of url.searchParams.getAll('sub')) {
			...
			body.outbounds.filter((node) => ['vless', 'shadowsocks'].includes(node.type)).forEach((node) => nodes.push(node));
		}
		...
		const groups = countries
			.map((country) => {
				return {
					type: 'selector',
					tag: country.zh,
					outbounds: nodes.filter((node) => node.tag.includes(country.emoji)).map((node) => node.tag),
				} satisfies Selector;
			})
			...
			outbounds: [
				{
					type: 'selector',
					tag: 'Proxy',
					outbounds: ['Direct', 'Auto', 'Block', ...groups.map((group) => group.tag)],
					default: '日本',
				},
				{
					type: 'urltest',
